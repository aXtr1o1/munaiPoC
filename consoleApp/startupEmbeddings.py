...
def generate_embeddings_parallel(df, batch_size=50):
    ...
    with ThreadPoolExecutor() as executor:
        results = list(tqdm(executor.map(get_batch_embeddings, batches), total=len(batches), desc="Generating Embeddings", colour='red'))
    for result in results:
        embeddings.extend(result)
    embeddings = np.array(embeddings)
    np.save('embeddingData/startupEmbeddings.npy', embeddings)
    return embeddings

df = load_csv("dataStuff/synthDataStartup.csv")
embeddings = generate_embeddings_parallel(df)
print("StartUp Embeddings Generated !")
